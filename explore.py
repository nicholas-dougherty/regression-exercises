...
def plot_variable_pairs(train, cols, hue=None):
    ...
    sns.pairplot(train[cols], hue=hue, kind="reg",plot_kws={'line_kws':{'color':'red'},
                                                            'scatter_kws': {'alpha': 0.1}})
    plt.show()
#|-----------------------------------------------------------------------------------------------|#
def months_to_years(train, validate, test):
    train['tenure_years'] = (train.tenure / 12).astype(int)
    validate['tenure_years'] = (validate.tenure / 12).astype(int)
    test['tenure_years'] = (test.tenure / 12).astype(int)
